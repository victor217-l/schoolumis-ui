...
class SeeCourses extends StatefulWidget {
  const SeeCourses({Key? key}) : super(key: key);

  @override
  State<SeeCourses> createState() => _SeeCoursesState();
}

class _SeeCoursesState extends State<SeeCourses> {

  final LectureService lectureService = LectureService();
  
  
  List<LectureCourse>? lectures;


  void fetchallcourse() async{
    lectures = await lectureService.fetchLectureteacehrcourse(context);
    setState(() {
            ...
            Padding(padding: EdgeInsets.symmetric(horizontal: 15),
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  lectures == null ? Center(child:CircularProgressIndicator(color: Globalvariables.colorforstablel,)) : ListView.builder(
                    itemCount: lectures!.length,
                      itemBuilder: (context,index) {
                      final lecturess = lectures![index];
                    return Column(
                      children: [
                        InkWell(                                                                                   //coursename,yearofcourse
                          onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_)=>
                              SeeStudentTakingCourse(coursename: lecturess.nameofcourse, year: lecturess.year, ) )),
                          child:Row(
                            children: [
