...
import 'package:schoolumis/features/model/worsi.dart';
import 'package:schoolumis/features/superadmin/addworshipcentr.dart';
...
class Allworshipr extends StatefulWidget {
  const Allworshipr({Key? key}) : super(key: key);

  @override
  State<Allworshipr> createState() => _AllworshiprState();
}

class _AllworshiprState extends State<Allworshipr> {


  Future<List<Worshipcenter>> fetchHalls() async {
    SharedPreferences prefss = await SharedPreferences.getInstance();
    final studentto = prefss.getString("studenttoken");

    List<Worshipcenter> _allworsip = [];

    try{

      http.Response res = await http.get(Uri.parse(
          "https://universitymanagem.onrender.com/superadmin/worshipcon"),
      ...
      httpErrorHandler(response: res, context: context, onSuccess: () {
        for(int i = 0; i < jsonDecode(res.body)['list'].length; i++ ){
          _allworsip.add(Worshipcenter.fromJson(jsonEncode(jsonDecode(res.body)['list'][i])));
        }
  ...
  List<Worshipcenter>? hallwprshi;
  late List<Worshipcenter> selectedHalls;


  void fetch() async {
    hallwprshi = await fetchHalls();
    setState(() {
  ...
  void onSelectedRow(bool selected, Worshipcenter  hallwprshi) {
    if (selected) {
      setState(() {
        selectedHalls.add(hallwprshi);
        // sendhall(student.hallofresidence, hall.firstname);

      });
    } else {
      setState(() {
        selectedHalls.remove(hallwprshi);
      });
    }
  }
  ...
  DataTable databody() {
    if(hallwprshi == null){
      return   DataTable(columns: [
        ...
        DataColumn(label: Text("Worship centre"),
            numeric: false,
            tooltip: "Halladmin",
            onSort: (columnIndex, ascendeing) {}
        )
      ...
      return DataTable(columns: [
        ...
        DataColumn(label: Text("Worship centre "),
            ...
            (halwl) =>
            DataRow(
              selected: selectedHalls.contains(halwl),
              onSelectChanged: (bool? selected) {
                print("Onselect");
                onSelectedRow(selected!, halwl);
              },
              cells: [
                DataCell(
                  Text(halwl.name,),
                  onTap: () {
                    print('Selected ${halwl.name}');
                  },
                ),
                DataCell(
                    Text(halwl.email),
                    onTap: () {
                      print('Selected ${halwl.email}');
                    }
                ),
                DataCell(
                    Text(halwl.password),
                    onTap: () {
                      print('Selected ${halwl.password}');
                    }
                ),

                DataCell(
                    Text(halwl.worshipcentername),
                    onTap: () {
                      print('Selected ${halwl.worshipcentername}');
                    }
                ),
                ...
                Padding(padding: EdgeInsets.all(13),
                  child: Column(
                    children: [
                      ...
                      InkWell(
                          onTap: () {
                            Navigator.of(context).push(MaterialPageRoute(builder: (_) => AddWorshipcentre()));
                          },

                          child: Text("Add Worship")),
                      SizedBox(height: 10,),
                      Center(
                        child: databody(),
                      ),
